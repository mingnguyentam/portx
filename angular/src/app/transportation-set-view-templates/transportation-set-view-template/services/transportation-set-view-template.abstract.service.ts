...
} from '../../../proxy/transportation-set-view-templates/models';
...
export abstract class AbstractTransportationSetViewTemplateViewService {
  protected readonly proxyService = inject(TransportationSetViewTemplateService);
  ...
  data: PagedResultDto<TransportationSetViewTemplateDto> = {
    items: [],
    totalCount: 0,
  };

  selectionType = SelectionType;
  selected = signal<TransportationSetViewTemplateDto[]>([]);
  allSelected = signal(false);
  selectedCount = computed(() => this.selected().length);

  filters = {} as GetTransportationSetViewTemplatesInput;
  ...
  hookToQuery() {
    ...
    const setData = (list: PagedResultDto<TransportationSetViewTemplateDto>) => {
      this.data = list;
  ...
  clearFilters() {
    this.filters = {} as GetTransportationSetViewTemplatesInput;
    this.list.get();
  }
  ...
  exportToExcel() {
    ...
    this.proxyService
      ...
      .subscribe(result => {
        this.abpWindowService.downloadBlob(result, 'TransportationSetViewTemplate.xlsx');
      });
  }
}
