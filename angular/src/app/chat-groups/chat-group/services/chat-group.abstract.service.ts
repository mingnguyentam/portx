...
import type { GetChatGroupsInput, ChatGroupDto } from '../../../proxy/chat-groups/models';
import { ChatGroupService } from '../../../proxy/chat-groups/chat-group.service';

export abstract class AbstractChatGroupViewService {
  protected readonly proxyService = inject(ChatGroupService);
  ...
  data: PagedResultDto<ChatGroupDto> = {
    items: [],
    totalCount: 0,
  };

  selectionType = SelectionType;
  selected = signal<ChatGroupDto[]>([]);
  allSelected = signal(false);
  selectedCount = computed(() => this.selected().length);

  filters = {} as GetChatGroupsInput;
  ...
  hookToQuery() {
    ...
    const setData = (list: PagedResultDto<ChatGroupDto>) => {
      this.data = list;
  ...
  clearFilters() {
    this.filters = {} as GetChatGroupsInput;
    this.list.get();
  }
  ...
  exportToExcel() {
    ...
    this.proxyService
      ...
      .subscribe(result => {
        this.abpWindowService.downloadBlob(result, 'ChatGroup.xlsx');
      });
  }
}
