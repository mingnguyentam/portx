...
import type { GetLocationsInput, LocationDto } from '../../../proxy/locations/models';
import { LocationService } from '../../../proxy/locations/location.service';

export abstract class AbstractLocationViewService {
  protected readonly proxyService = inject(LocationService);
  ...
  selected = signal<LocationDto[]>([]);
  allSelected = signal(false);
  selectedCount = computed(() => this.selected().length);

  filters = {} as GetLocationsInput;
  ...
  clearFilters() {
    this.filters = {} as GetLocationsInput;
    this.list.get();
  }
  ...
  exportToExcel() {
    ...
    this.proxyService
      ...
      .subscribe(result => {
        this.abpWindowService.downloadBlob(result, 'Location.xlsx');
      });
  }
}
