...
} from '../../../proxy/transportation-sets/models';
import { TransportationSetService } from '../../../proxy/transportation-sets/transportation-set.service';

export abstract class AbstractTransportationSetViewService {
  protected readonly proxyService = inject(TransportationSetService);
  ...
  data: PagedResultDto<TransportationSetDto> = {
    items: [],
    totalCount: 0,
  };

  selectionType = SelectionType;
  selected = signal<TransportationSetDto[]>([]);
  allSelected = signal(false);
  selectedCount = computed(() => this.selected().length);

  filters = {} as GetTransportationSetsInput;
  ...
  hookToQuery() {
    ...
    const setData = (list: PagedResultDto<TransportationSetDto>) => {
      this.data = list;
  ...
  clearFilters() {
    this.filters = {} as GetTransportationSetsInput;
    this.list.get();
  }
  ...
  exportToExcel() {
    ...
    this.proxyService
      ...
      .subscribe(result => {
        this.abpWindowService.downloadBlob(result, 'TransportationSet.xlsx');
      });
  }
}
