...
} from '../../../proxy/shipping-workflow-display-items/models';
...
export abstract class AbstractShippingWorkflowDisplayItemViewService {
  protected readonly proxyService = inject(ShippingWorkflowDisplayItemService);
  ...
  data: PagedResultDto<ShippingWorkflowDisplayItemDto> = {
    items: [],
    totalCount: 0,
  };

  selectionType = SelectionType;
  selected = signal<ShippingWorkflowDisplayItemDto[]>([]);
  allSelected = signal(false);
  selectedCount = computed(() => this.selected().length);

  filters = {} as GetShippingWorkflowDisplayItemsInput;
  ...
  hookToQuery() {
    ...
    const setData = (list: PagedResultDto<ShippingWorkflowDisplayItemDto>) => {
      this.data = list;
  ...
  clearFilters() {
    this.filters = {} as GetShippingWorkflowDisplayItemsInput;
    this.list.get();
  }
  ...
  exportToExcel() {
    ...
    this.proxyService
      ...
      .subscribe(result => {
        this.abpWindowService.downloadBlob(result, 'ShippingWorkflowDisplayItem.xlsx');
      });
  }
}
