...
import type {
  ...
  ShippingWorkflowDto,
} from '../../../proxy/shipping-workflows/models';
import { ShippingWorkflowService } from '../../../proxy/shipping-workflows/shipping-workflow.service';

export abstract class AbstractShippingWorkflowViewService {
  protected readonly proxyService = inject(ShippingWorkflowService);
  ...
  data: PagedResultDto<ShippingWorkflowDto> = {
    items: [],
    totalCount: 0,
  };

  selectionType = SelectionType;
  selected = signal<ShippingWorkflowDto[]>([]);
  allSelected = signal(false);
  selectedCount = computed(() => this.selected().length);

  filters = {} as GetShippingWorkflowsInput;
  ...
  hookToQuery() {
    ...
    const setData = (list: PagedResultDto<ShippingWorkflowDto>) => {
      this.data = list;
  ...
  clearFilters() {
    this.filters = {} as GetShippingWorkflowsInput;
    this.list.get();
  }
  ...
  exportToExcel() {
    ...
    this.proxyService
      ...
      .subscribe(result => {
        this.abpWindowService.downloadBlob(result, 'ShippingWorkflow.xlsx');
      });
  }
}
