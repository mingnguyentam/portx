...
} from '../../../proxy/combined-lane-items/models';
import { CombinedLaneItemService } from '../../../proxy/combined-lane-items/combined-lane-item.service';

export abstract class AbstractCombinedLaneItemViewService {
  protected readonly proxyService = inject(CombinedLaneItemService);
  ...
  data: PagedResultDto<CombinedLaneItemWithNavigationPropertiesDto> = {
    items: [],
    totalCount: 0,
  };

  selectionType = SelectionType;
  selected = signal<CombinedLaneItemWithNavigationPropertiesDto[]>([]);
  allSelected = signal(false);
  selectedCount = computed(() => this.selected().length);

  filters = {} as GetCombinedLaneItemsInput;
  ...
  protected bulkDeleteRequest() {
    const ids = this.selected().map(({ combinedLaneItem: { id } }) => id);
  ...
  delete(record: CombinedLaneItemWithNavigationPropertiesDto) {
    this.confirmationService
      ...
      .pipe(
        ...
        switchMap(() => this.proxyService.delete(record.combinedLaneItem.id)),
      )
  ...
  hookToQuery() {
    ...
    const setData = (list: PagedResultDto<CombinedLaneItemWithNavigationPropertiesDto>) => {
      this.data = list;
  ...
  clearFilters() {
    this.filters = {} as GetCombinedLaneItemsInput;
    this.list.get();
  }
  ...
  exportToExcel() {
    ...
    this.proxyService
      ...
      .subscribe(result => {
        this.abpWindowService.downloadBlob(result, 'CombinedLaneItem.xlsx');
      });
  }
}
