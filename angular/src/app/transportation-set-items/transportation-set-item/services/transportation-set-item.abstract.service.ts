...
} from '../../../proxy/transportation-set-items/models';
import { TransportationSetItemService } from '../../../proxy/transportation-set-items/transportation-set-item.service';

export abstract class AbstractTransportationSetItemViewService {
  protected readonly proxyService = inject(TransportationSetItemService);
  ...
  data: PagedResultDto<TransportationSetItemDto> = {
    items: [],
    totalCount: 0,
  };

  selectionType = SelectionType;
  selected = signal<TransportationSetItemDto[]>([]);
  allSelected = signal(false);
  selectedCount = computed(() => this.selected().length);

  filters = {} as GetTransportationSetItemsInput;
  ...
  hookToQuery() {
    ...
    const setData = (list: PagedResultDto<TransportationSetItemDto>) => {
      this.data = list;
  ...
  clearFilters() {
    this.filters = {} as GetTransportationSetItemsInput;
    this.list.get();
  }
  ...
  exportToExcel() {
    ...
    this.proxyService
      ...
      .subscribe(result => {
        this.abpWindowService.downloadBlob(result, 'TransportationSetItem.xlsx');
      });
  }
}
