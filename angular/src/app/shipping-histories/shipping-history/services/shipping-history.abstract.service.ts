...
import type {
  ...
  ShippingHistoryDto,
} from '../../../proxy/shipping-histories/models';
import { ShippingHistoryService } from '../../../proxy/shipping-histories/shipping-history.service';

export abstract class AbstractShippingHistoryViewService {
  protected readonly proxyService = inject(ShippingHistoryService);
  ...
  data: PagedResultDto<ShippingHistoryDto> = {
    items: [],
    totalCount: 0,
  };

  selectionType = SelectionType;
  selected = signal<ShippingHistoryDto[]>([]);
  allSelected = signal(false);
  selectedCount = computed(() => this.selected().length);

  filters = {} as GetShippingHistoriesInput;
  ...
  hookToQuery() {
    ...
    const setData = (list: PagedResultDto<ShippingHistoryDto>) => {
      this.data = list;
  ...
  clearFilters() {
    this.filters = {} as GetShippingHistoriesInput;
    this.list.get();
  }
  ...
  exportToExcel() {
    ...
    this.proxyService
      ...
      .subscribe(result => {
        this.abpWindowService.downloadBlob(result, 'ShippingHistory.xlsx');
      });
  }
}
