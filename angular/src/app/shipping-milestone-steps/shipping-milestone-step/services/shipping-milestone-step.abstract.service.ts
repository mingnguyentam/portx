...
} from '../../../proxy/shipping-milestone-steps/models';
import { ShippingMilestoneStepService } from '../../../proxy/shipping-milestone-steps/shipping-milestone-step.service';

export abstract class AbstractShippingMilestoneStepViewService {
  protected readonly proxyService = inject(ShippingMilestoneStepService);
  ...
  data: PagedResultDto<ShippingMilestoneStepDto> = {
    items: [],
    totalCount: 0,
  };

  selectionType = SelectionType;
  selected = signal<ShippingMilestoneStepDto[]>([]);
  allSelected = signal(false);
  selectedCount = computed(() => this.selected().length);

  filters = {} as GetShippingMilestoneStepsInput;
  ...
  hookToQuery() {
    ...
    const setData = (list: PagedResultDto<ShippingMilestoneStepDto>) => {
      this.data = list;
  ...
  clearFilters() {
    this.filters = {} as GetShippingMilestoneStepsInput;
    this.list.get();
  }
  ...
  exportToExcel() {
    ...
    this.proxyService
      ...
      .subscribe(result => {
        this.abpWindowService.downloadBlob(result, 'ShippingMilestoneStep.xlsx');
      });
  }
}
