...
} from '../../../proxy/transportation-set-item-groups/models';
import { TransportationSetItemGroupService } from '../../../proxy/transportation-set-item-groups/transportation-set-item-group.service';

export abstract class AbstractTransportationSetItemGroupViewService {
  protected readonly proxyService = inject(TransportationSetItemGroupService);
  ...
  data: PagedResultDto<TransportationSetItemGroupDto> = {
    items: [],
    totalCount: 0,
  };

  selectionType = SelectionType;
  selected = signal<TransportationSetItemGroupDto[]>([]);
  allSelected = signal(false);
  selectedCount = computed(() => this.selected().length);

  filters = {} as GetTransportationSetItemGroupsInput;
  ...
  hookToQuery() {
    ...
    const setData = (list: PagedResultDto<TransportationSetItemGroupDto>) => {
      this.data = list;
  ...
  clearFilters() {
    this.filters = {} as GetTransportationSetItemGroupsInput;
    this.list.get();
  }
  ...
  exportToExcel() {
    ...
    this.proxyService
      ...
      .subscribe(result => {
        this.abpWindowService.downloadBlob(result, 'TransportationSetItemGroup.xlsx');
      });
  }
}
