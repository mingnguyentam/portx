...
import type { GetCombinedLanesInput, CombinedLaneDto } from '../../../proxy/combined-lanes/models';
import { CombinedLaneService } from '../../../proxy/combined-lanes/combined-lane.service';

export abstract class AbstractCombinedLaneViewService {
  protected readonly proxyService = inject(CombinedLaneService);
  ...
  data: PagedResultDto<CombinedLaneDto> = {
    items: [],
    totalCount: 0,
  };

  selectionType = SelectionType;
  selected = signal<CombinedLaneDto[]>([]);
  allSelected = signal(false);
  selectedCount = computed(() => this.selected().length);

  filters = {} as GetCombinedLanesInput;
  ...
  hookToQuery() {
    ...
    const setData = (list: PagedResultDto<CombinedLaneDto>) => {
      this.data = list;
  ...
  clearFilters() {
    this.filters = {} as GetCombinedLanesInput;
    this.list.get();
  }
  ...
  exportToExcel() {
    ...
    this.proxyService
      ...
      .subscribe(result => {
        this.abpWindowService.downloadBlob(result, 'CombinedLane.xlsx');
      });
  }
}
