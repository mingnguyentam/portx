...
import type { GetShippingFormsInput, ShippingFormDto } from '../../../proxy/shipping-forms/models';
import { ShippingFormService } from '../../../proxy/shipping-forms/shipping-form.service';

export abstract class AbstractShippingFormViewService {
  protected readonly proxyService = inject(ShippingFormService);
  ...
  data: PagedResultDto<ShippingFormDto> = {
    items: [],
    totalCount: 0,
  };

  selectionType = SelectionType;
  selected = signal<ShippingFormDto[]>([]);
  allSelected = signal(false);
  selectedCount = computed(() => this.selected().length);

  filters = {} as GetShippingFormsInput;
  ...
  hookToQuery() {
    ...
    const setData = (list: PagedResultDto<ShippingFormDto>) => {
      this.data = list;
  ...
  clearFilters() {
    this.filters = {} as GetShippingFormsInput;
    this.list.get();
  }
  ...
  exportToExcel() {
    ...
    this.proxyService
      ...
      .subscribe(result => {
        this.abpWindowService.downloadBlob(result, 'ShippingForm.xlsx');
      });
  }
}
