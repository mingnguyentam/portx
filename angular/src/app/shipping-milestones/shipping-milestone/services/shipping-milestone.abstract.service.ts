...
} from '../../../proxy/shipping-milestones/models';
import { ShippingMilestoneService } from '../../../proxy/shipping-milestones/shipping-milestone.service';

export abstract class AbstractShippingMilestoneViewService {
  protected readonly proxyService = inject(ShippingMilestoneService);
  ...
  data: PagedResultDto<ShippingMilestoneDto> = {
    items: [],
    totalCount: 0,
  };

  selectionType = SelectionType;
  selected = signal<ShippingMilestoneDto[]>([]);
  allSelected = signal(false);
  selectedCount = computed(() => this.selected().length);

  filters = {} as GetShippingMilestonesInput;
  ...
  hookToQuery() {
    ...
    const setData = (list: PagedResultDto<ShippingMilestoneDto>) => {
      this.data = list;
  ...
  clearFilters() {
    this.filters = {} as GetShippingMilestonesInput;
    this.list.get();
  }
  ...
  exportToExcel() {
    ...
    this.proxyService
      ...
      .subscribe(result => {
        this.abpWindowService.downloadBlob(result, 'ShippingMilestone.xlsx');
      });
  }
}
