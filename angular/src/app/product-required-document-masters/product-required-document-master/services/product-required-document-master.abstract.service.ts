...
} from '../../../proxy/product-required-document-masters/models';
...
export abstract class AbstractProductRequiredDocumentMasterViewService {
  protected readonly proxyService = inject(ProductRequiredDocumentMasterService);
  ...
  data: PagedResultDto<ProductRequiredDocumentMasterWithNavigationPropertiesDto> = {
    items: [],
    totalCount: 0,
  };

  selectionType = SelectionType;
  selected = signal<ProductRequiredDocumentMasterWithNavigationPropertiesDto[]>([]);
  allSelected = signal(false);
  selectedCount = computed(() => this.selected().length);

  filters = {} as GetProductRequiredDocumentMastersInput;
  ...
  protected bulkDeleteRequest() {
    const ids = this.selected().map(({ productRequiredDocumentMaster: { id } }) => id);
  ...
  delete(record: ProductRequiredDocumentMasterWithNavigationPropertiesDto) {
    this.confirmationService
      ...
      .pipe(
        ...
        switchMap(() => this.proxyService.delete(record.productRequiredDocumentMaster.id)),
      )
  ...
  hookToQuery() {
    ...
    const setData = (
      list: PagedResultDto<ProductRequiredDocumentMasterWithNavigationPropertiesDto>,
    ) => {
      this.data = list;
  ...
  clearFilters() {
    this.filters = {} as GetProductRequiredDocumentMastersInput;
    this.list.get();
  }
  ...
  exportToExcel() {
    ...
    this.proxyService
      ...
      .subscribe(result => {
        this.abpWindowService.downloadBlob(result, 'ProductRequiredDocumentMaster.xlsx');
      });
  }
}
